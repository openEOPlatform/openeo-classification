...
def _get_coords(shp):
    ...
    coord_info = utm.from_latlon(*p[::-1])
...
def read_f(directory:str = "resources/reference_data/") -> pd.DataFrame:
    ...
    esyrce = gpd.read_file(directory+"2019_ESP_ESYRCE_POLY_111.gpkg")[["CT","geometry"]]
    esyrce["year"] = 2019
    esyrce["ref_id"] = "2019_ESP_ESYRCE_POLY_111"
    esyrce[["zonenumber","zoneletter"]] = esyrce["geometry"].centroid.apply(_get_coords)

    mapping = { 1100: 1110, 1500: 1510, 1900: 1910}
    esyrce["CT"] = esyrce["CT"].apply(lambda x: mapping[x] if x in mapping else x)
    esyrce = esyrce[esyrce["CT"]!=9120]

    pd.set_option('display.max_rows', None)
    print(esyrce.groupby("CT").count())

    files = [esyrce]
    fns = ["2019_ESP_ESYRCE_POLY_111.gpkg"]
    print("Finished loading data.")

    return esyrce, pd.DataFrame(columns=["CT","geometry","year","ref_id","zonenumber","zoneletter"]), fns
...
class SamplePolygons():
    ...
    def sample_and_store_polygons(self):
        ...
        for crop_id in crop_ids:
            self.sample_and_store_id(crop_id, output_folder=self.output_folder  / "crops_of_interest_spain", tot_samp_lpis=self.tot_samp_crops_lpis, tot_samp_lucas=self.tot_samp_crops_lucas)
        for crop_id in other_crop_ids:
            self.sample_and_store_id(crop_id, output_folder=self.output_folder / "other_crops_spain", tot_samp_lpis=self.tot_samp_other_lpis, tot_samp_lucas=self.tot_samp_other_lucas)
