...
class SamplePolygons():
    def __init__(self, crop_list, zones, years, input_df, aez_stratification, output_folder = Path("resources")/ "training_data", 
        tot_samp_crops=500, tot_samp_other=200, repeat_per_sample=3):
        ...
        self.tot_samp_crops = tot_samp_crops
        self.tot_samp_other = tot_samp_other
        self.repeat_per_sample = repeat_per_sample

        if self.input_df is None:
            self.input_df, self.fns = read_f()


    def sample_polygons(self, crop_id, tot_samp=2000) -> pd.DataFrame:
        """
        ...
        """
        if self.repeat_per_sample < 1:
            raise ValueError("Repeat per sample has a minimum value of 1!")

        crop_df = self.input_df[self.input_df["CT"] == crop_id]
        ...
        tot_df = pd.concat([crop_df]+[crop_df.copy()]*(self.repeat_per_sample-1), ignore_index=True)
        if tot_samp > len(tot_df):
            print("The amount of {} samples you want is more than {} times the total amount of samples, {}. Hence the amount of features sampled for crop {} is: {}".format(
                crop_id, self.repeat_per_sample, len(crop_df), crop_id, len(tot_df)))
            tot_samp = len(tot_df)
    ...
    def sample_and_store_id(self,crop_id, output_folder, tot_samp):
        print("Starting to sample polygons for id {}".format(crop_id))
        crop = self.sample_polygons(crop_id=crop_id, tot_samp=tot_samp)
        crop["sample_polygon"] = crop["geometry"].apply(self._extract_point_from_polygon)
        if not crop.empty:
    ...
    def sample_and_store_polygons(self):
        ...
        for crop_id in crop_ids:
            self.sample_and_store_id(crop_id, output_folder=self.output_folder  / "crops_of_interest", tot_samp=self.tot_samp_crops)
        for crop_id in other_crop_ids:
            self.sample_and_store_id(crop_id, output_folder=self.output_folder / "other_crops", tot_samp=self.tot_samp_other)
