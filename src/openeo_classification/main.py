...
from sample import sample_polygons, read_f
...
crop_ids, other_crop_ids = sample_and_store_polygons(ids=crop_list, zones=zones, years=years, input_df=input_data, output_folder="resources/training_data/")

## Load or create the file that will monitor job statistics
df = create_or_load_job_statistics(path = "resources/training_data/job_statistics.csv")

for year in years:
    features = load_features(year)
    for i in ids:
        for fnp in glob.glob("training_data/crops_of_interest/sampleable_polygons_year"+str(year)+"_zone"+zone+"_id"+str(i)+"*.json"):
            if fnp in df["fp"].tolist():
                continue
            while True:
                if len(df[(df["status"]=="running") | (df["status"]=="queued")]) < 2:
                    ...
                    df.to_csv("training_data/job_statistics.csv",index=False)
                    break
                else:
                    time.sleep(60)
